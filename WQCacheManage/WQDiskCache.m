...
static int64_t _WQDiskSpaceFree() {
    NSError *error = nil;
...
// String's md5 hash.
static NSString *_WQNSStringMD5(NSString *string) {
...
static void _WQDiskCacheInitGlobal() {
...
static WQDiskCache *_WQDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _WQDiskCacheInitGlobal();
...
static void _WQDiskCacheSetGlobal(WQDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _WQDiskSpaceFree();
...
- (NSString *)_filenameForKey:(NSString *)key {
    ...
    if (!filename) filename = _WQNSStringMD5(key);;
    return filename;
}
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"WQDiskCache init error" reason:@"WQDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    WQDiskCache *globalCache = _WQDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    WQKVStorageType type;
    if (threshold == 0) {
        type = WQKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = WQKVStorageTypeSQLite;
    } else {
        type = WQKVStorageTypeMixed;
    }
    
    WQKVStorage *kv = [[WQKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    WQKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [WQDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [WQDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != WQKVStorageTypeSQLite) {
