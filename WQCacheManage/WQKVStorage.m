...
static UIApplication *_WQSharedApplication() {
...
@implementation WQKVStorageItem
@end

@implementation WQKVStorage {
...
- (WQKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    WQKVStorageItem *item = [WQKVStorageItem new];
...
- (WQKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    WQKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            WQKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"WQKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:WQKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(WQKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"WQKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > WQKVStorageTypeMixed) {
        NSLog(@"WQKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_WQSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_WQSharedApplication() endBackgroundTask:taskID];
    }
}
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == WQKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != WQKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case WQKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case WQKVStorageTypeFile:
        case WQKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (WQKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (WQKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    WQKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (WQKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    WQKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
